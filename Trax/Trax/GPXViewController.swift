...
class GPXViewController: UIViewController, MKMapViewDelegate {
    ...
    func mapView(mapView: MKMapView!, didSelectAnnotationView view: MKAnnotationView!) {
        if let waypoint = view.annotation as? GPX.Waypoint {
            if let thumbnamilImageView = view.leftCalloutAccessoryView as? UIImageView {
                if let imageData = NSData(contentsOfURL: waypoint.thumbnailURL!) {
                    if let image = UIImage(data: imageData) {
                        thumbnamilImageView.image = image
                    }
                }
            }
        }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == Constants.ShowImageSegue {
            if let waypoint = (send as? MKAnnotationView)?.annotation as? GPX.Waypoint {
